...
jQuery(document).ready(function() {
	...
	jQuery( "#menu_gen" ).click(function() {
	   jQuery("#tarali_sel").val('bloc_gen');
	...
	jQuery( "#menu_hp" ).click(function() {
	   jQuery("#tarali_sel").val('bloc_hp');
	   jQuery(".wrap-admin").css("backgroundColor",jQuery(this).css("backgroundColor"));
       jQuery("#bloc_hp").fadeIn();
	   
    });
	jQuery( "#menu_sli" ).click(function() {
	   jQuery("#tarali_sel").val('bloc_sli');
	   jQuery(".wrap-admin").css("backgroundColor",jQuery(this).css("backgroundColor"));
       jQuery("#bloc_sli").fadeIn();
    });
	jQuery( "#menu_goo" ).click(function() {
	   jQuery("#tarali_sel").val('bloc_goo');
	   jQuery(".wrap-admin").css("backgroundColor",jQuery(this).css("backgroundColor"));
       jQuery("#bloc_goo").fadeIn();
    });
	jQuery( "#menu_sn" ).click(function() {
	   jQuery("#tarali_sel").val('bloc_sn');
	   jQuery(".wrap-admin").css("backgroundColor",jQuery(this).css("backgroundColor"));
       jQuery("#bloc_sn").fadeIn();
    });
	
	
	if ((jQuery("#tarali_sel").val()=="bloc_gen")|| (jQuery("#tarali_sel").val()=="")){
		jQuery( "#menu_gen" ).trigger( "click" );
	}else if ((jQuery("#tarali_sel").val()=="bloc_hp")){
		jQuery( "#menu_hp" ).trigger( "click" );
	}else if ((jQuery("#tarali_sel").val()=="bloc_sli")){
		jQuery( "#menu_sli" ).trigger( "click" );
	}else if ((jQuery("#tarali_sel").val()=="bloc_goo")){
...
jQuery('.my-color-field').wpColorPicker(myOptions);
 ...
 jQuery("#tarali_ff").joliSelect(
   {
    ...
    jQuery("#tarali_title_ff").joliSelect(
   {
    'width'          : '160',
	'autocomplete'   : false,
	 'bkdColor'      : '#fcfcfc',
    ...
    jQuery("#tarali_header_ff").joliSelect(
   {
    'width'          : '160',
	'autocomplete'   : false,
	 'bkdColor'      : '#fcfcfc',
   ...
   jQuery("#tarali_sl_ef").joliSelect(
   {
    'width'          : '160',
	'autocomplete'   : false,
	 'bkdColor'      : '#fcfcfc',
   ...
   jQuery("#tarali_sl_cpt").joliSelect(
   {
    'width'          : '160',
	'autocomplete'   : false,
	 'bkdColor'      : '#fcfcfc',
 ...
 jQuery('.tarali_img_btn').click(function() {
...
window.send_to_editor = function(html) {
 ...
 jQuery("#"+formfield+"_lnk").attr('src',imgurl);
 jQuery("#"+formfield+"_id").val(id);
 if (tableparent){
	   tableparent.next("table").fadeIn();
 }
 var id=formfield.replace('tarali_hp_sl_im', '');
 var nextdel= jQuery("#delete_tarali_hp_sl_im"+(parseInt(id)-1)+"");		
		 if (nextdel){  
	      nextdel.fadeOut();
         }
		 var prevdel=jQuery("#delete_tarali_hp_sl_im"+(parseInt(id))+"");	
        if (prevdel){  
	      prevdel.fadeIn();
         }		
 tb_remove();
...
jQuery('.tarali_hp_sl_im_delete').click(function() {
    if (confirm ("Delete ? ")){
		 var id=jQuery(this).attr("id");
		 id=id.replace('delete_tarali_hp_sl_im', '');
		 jQuery("#tarali_hp_sl_im"+id+"").attr("value", "");
		 jQuery("#tarali_hp_sl_im"+id+"_id").attr("value", "");
		 jQuery("#tarali_hp_sl_lk"+id+"").attr("value", "");
		 jQuery("#tarali_hp_sl_txt"+id+"").attr("value", "");
		 jQuery("#tarali_hp_sl_title"+id+"").attr("value", "");
		 jQuery("#tarali_hp_sl_im"+id+"_lnk").attr("src", "");
		 jQuery(this).fadeOut();
		...
		var nexttable= jQuery("#table_"+(parseInt(id)+1)+"");		
		 ...
		 var prevdel=jQuery("#delete_tarali_hp_sl_im"+(parseInt(id)-1)+"");	
        if (prevdel){  
	      prevdel.fadeIn();
         }		 
	} 
